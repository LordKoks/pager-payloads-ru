...
if [[ ! -f "$CONFIG_FILE" ]]; then
    ERROR_DIALOG "Hashtopolis Upload - Configuration file не найден. Please create config.sh file."
    exit 1
fi
...
# Note: Ensure tcpdump is установлен on the system running this
SSID=$(tcpdump -r "$PCAP" -e -I -s 256 2>/dev/null \
  | sed -n 's/.*Beacon (\([^)]*\)).*/\1/p' \
  | head -n 1)

# Fallback if SSID не найден or empty
if [[ -z "$SSID" ]]; then
    SSID="UNKNOWN_SSID"
fi
...
if echo "$EXISTING_LISTS" | grep -Fiq "$MAC_CHECK"; then
    ALERT "Hashtopolis Upload - ПРОПУЩЕНО! Handshake for SSID: $SSID ($MAC_CHECK) already exists in Hashtopolis. Skipping upload."
    exit 0
fi
...
if [[ ! -f "$_ALERT_HANDSHAKE_HASHCAT_PATH" ]]; then
    ERROR_DIALOG "Hashtopolis Upload - Hashcat file не найден at $_ALERT_HANDSHAKE_HASHCAT_PATH"
    exit 1
fi
