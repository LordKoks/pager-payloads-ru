...
if [[ -z "$HASHTOPOLIS_URL" ]]; then
    echo "ОШИБКА: HASHTOPOLIS_URL не задан в config.sh"
    exit 1
fi

if [[ -z "$API_KEY" ]]; then
    echo "ОШИБКА: API_KEY не задан в config.sh"
    exit 1
fi

if [[ -z "$PRETASK_ID" ]]; then
    echo "ОШИБКА: PRETASK_ID не задан в config.sh"
    exit 1
fi

if [[ -z "$CRACKER_VERSION_ID" ]]; then
    echo "ОШИБКА: CRACKER_VERSION_ID не задан в config.sh"
    exit 1
fi
