...
# Автор: Unit981
# Описание: Обновлённый алерт на захват хэндшейка, проверяет дубликаты по MAC точки доступа и удаляет дубликатные хэндшейки для чистоты.
# Версия: 1.0

# Установка папки и переменных
...
# Запасной вариант, если SSID не найден
[ -n "$SSID" ] || SSID="СКРЫТАЯ_СЕТЬ"
