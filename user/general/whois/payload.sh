...
setup() {
    ...
    if ! command -v whois >/dev/null 2>&1; then
        ...
        if ! command -v whois >/dev/null 2>&1; then
            ...
            LOG "Exiting - whois is required but could not be установлен"
            exit 1
        fi
    fi
}
...
check_network() {
    ...
    if [ "$has_ip" = false ]; then
        LOG "ERROR: Действительный IP-адрес не обнаружен"
        ERROR_DIALOG "Действительный IP-адрес не обнаружен. This utility requires a valid IP address. Please ensure the device is in client mode and connected to a network."
