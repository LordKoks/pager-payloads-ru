...
if read_saved_config; then
    ...
    LOG "Hashtopolis Upload - Найдено saved config."
...
# =============================================================================
# CONFIGURATION ХРАНИЛИЩЕ (OPTIONAL UPDATE)
# =============================================================================
...
if [[ "$FORCE_CONFIG_UPDATE" == true ]]; then
    ...
    # Only prompt to update when config.sh differs from saved config and не sample data.
...
if [[ ! -d "$HANDSHAKE_DIR" ]]; then
    ERROR_DIALOG "Hashtopolis Upload - Handshake directory не найден: $HANDSHAKE_DIR"
    exit 1
fi
