...
if ! ping -c 1 -W 3 8.8.8.8 > /dev/null 2>&1; then
    LOG red "ERROR: Маршрут к хосту не найден"
    ERROR_DIALOG "Маршрут к хосту не найден. Убедитесь, что устройство имеет доступ в Интернет to perform a speed test."
...
# Check if speedtest-cli is установлен, if not download it
if [ ! -f "$SPEEDTEST_CLI_PATH" ]; then
    LOG yellow "speedtest-cli не найден. Downloading..."
    ...
    if command -v curl >/dev/null 2>&1; then
        ...
        if [ -n "$geo_data" ]; then
            ...
            if [ -n "$public_ip" ] && [ "$public_ip" != "null" ]; then
                ...
                echo "Публичный IP: не удалось получить"
            fi
        else
        ...
        if [ -n "$public_ip" ]; then
            echo "Public IP: $public_ip"
            echo "Геолокация: недоступна (требуется curl)"
        else
            echo "Публичный IP: не удалось получить"
        fi
    else
        echo "Публичный IP: недоступен (нет инструмента загрузки)"
    fi
    
    echo ""
    echo "=== ТЕСТ СКОРОСТИ RESULTS ==="
} > "$lootfile"
    STOP_SPINNER $spinner_id
    
    LOG green "Сеть intel gathered"
    LOG blue "Results will be dumped to ${LOOTDIR}"
else
    LOG blue "Loot generation disabled (results will only be displayed)"
fi

LOG cyan "Запуск теста скорости..."
...
if [ "$was_killed" = true ]; then
    LOG yellow "WARNING: Тест скорости was killed (likely OOM)"
    if [ "$generate_loot" = "yes" ]; then
        echo "" >> "$lootfile"
        echo "ПРИМЕЧАНИЕ: тест скорости был прерван во время выполнения" >> "$lootfile"
    fi
fi
...
if [ "$generate_loot" = "yes" ]; then
    ...
    speedtest_section=$(sed -n '/=== ТЕСТ СКОРОСТИ RESULTS ===/,$p' "$lootfile")
else
...
coordinates=""
if [ "$generate_loot" = "yes" ]; then
    geo_section=$(sed -n '/=== PUBLIC IP & GEOLOCATION ===/,/=== ТЕСТ СКОРОСТИ RESULTS ===/p' "$lootfile")
...
# Build alert message
alert_msg="Результаты теста скорости\n(speedtest.net)\n\n"
...
if [ -n "$ping_line" ] || [ -n "$download_line" ] || [ -n "$upload_line" ]; then
    ...
    ERROR_DIALOG "Тест скорости completed but no results found. Check logs for details."
fi
