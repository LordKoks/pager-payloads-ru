...
Downloads and installs payloads from a specific GitHub Pull Request in the WiFi Pineapple Pager payloads repository.

## Description

This payload allows you to test payloads from Pull Requests before they are merged into the main repository. It downloads the PR archive, extracts changed files from the `library/` directory, and installs them to your Pager's payload directory (`/mmc/root/payloads`).
...
- `GH_REPO`: Repository name (default: `wifipineapplepager-payloads`)
- `TARGET_DIR`: Installation directory (default: `/mmc/root/payloads`)
