...
Downloads and installs ringtones from a specific GitHub Pull Request in the WiFi Pineapple Pager ringtones repository.

## Description

This payload allows you to test ringtones from Pull Requests before they are merged into the main repository. It downloads the PR archive, extracts changed files from the `ringtones/` directory, and installs them to your Pager's ringtones directory (`/mmc/root/ringtones`).
...
- `GH_REPO`: Repository name (default: `wifipineapplepager-ringtones`)
- `TARGET_DIR`: Installation directory (default: `/mmc/root/ringtones`)
