...
dependency_check() {
  ...
  if ! which uqmi >/dev/null 2>&1; then
    LOG yellow "uqmi не найден, installing..."
    ...
    LOG green "uqmi установлен successfully"
  else
  	LOG green "Dependencies установлен"
  fi
}
...
configure_apn() {
  ...
  PROMPT "The Access Point Name (APN) address of your Mobile Сеть Operator must be configured for LTE access.\n\nPress any key to configure"
...
modem_check() {
  ...
  if lsusb | grep -q "ID 2c7c:0125 Quectel EG25-G"; then
    ...
    ERROR_DIALOG "Modem не найден. Connect the Hak5 eLiTE Mobile Broadband Adapter and try the LTE Configuration payload again."
    exit 0
  fi
}
...
lte_status() {
  ...
  if lsusb | grep -q "ID 2c7c:0125 Quectel EG25-G"; then
    ...
    LOG red "Modem не найден: Quectel EG25-G (2c7c:0125)"
    return 1
  fi
