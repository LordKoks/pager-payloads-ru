...
if [ ! -f ./mobile2gps ]; then
  LOG red "ERROR: mobile2gps binary не найден!"
...
while true; do
  ...
  if [ "$choice" = "UP" ]; then
    ...
    LOG ""
    LOG "Нажмите любую кнопку to continue."
    ...
    LOG ""
    LOG "Нажмите любую кнопку to return to the menu."
    __button=$(WAIT_FOR_INPUT)
