...
search_payloads() {
    ...
    if [ ! -d "$PAYLOAD_DIR" ]; then
        LED FAIL
        ERROR_DIALOG "Payload directory не найден: $PAYLOAD_DIR"
        exit 1
    fi
...
display_results() {
    ...
    if [ "$count" -eq 0 ]; then
        LOG "Сценарии не найдены matching your search."
        LED FAIL
        return 1
    fi
    
    LOG "Найдено $count matching payload(s):"
    LOG "================================"
...
view_payload_details() {
    ...
    if [ ! -f "$full_path" ]; then
        ERROR_DIALOG "Payload file не найден: $full_path"
        return 1
    fi
