...
if [ "$VPN_TYPE" -eq 1 ]; then
    ...
    if ! command -v wg >/dev/null 2>&1; then
        resp=$(CONFIRMATION_DIALOG "WireGuard не установлен!
...
Press OK to install.")
        if [ "$resp" = "$DUCKYSCRIPT_USER_CONFIRMED" ]; then
            ...
            SPINNER_STOP
            if ! command -v wg >/dev/null 2>&1; then
                ERROR_DIALOG "WireGuard install не удалась!
...
sections."
    ...
    [ ! -f "$WG_CONF" ] && { ERROR_DIALOG "Config не найден!
...
and [Peer] sections."; exit 1
    ...
    if ! command -v openvpn >/dev/null 2>&1; then
        resp=$(CONFIRMATION_DIALOG "OpenVPN не установлен!
...
Press OK to install.")
        if [ "$resp" = "$DUCKYSCRIPT_USER_CONFIRMED" ]; then
            ...
            SPINNER_STOP
            if ! command -v openvpn >/dev/null 2>&1; then
                ERROR_DIALOG "OpenVPN install не удалась!"; exit 1
            fi
        else
            exit 1
        fi
...
Or enter a custom path."
    ...
    [ ! -f "$OVPN_CONF" ] && { ERROR_DIALOG "Config не найден!
