...
LOG "Останавливаю службы goodportal..."
...
if [ -f /etc/nginx/nginx.conf.goodportal.bak ]; then
    ...
    LOG "  WARNING: Backup nginx.conf не найден, skipping restore"
fi
...
LOG yellow "NOTE: nginx and PHP packages will remain установлен. Running the goodportal Configure payload again will be MUCH faster next time!"
