...
# Dependencies:
#   - nginx (установлен if missing)
#   - php8-fpm (optional, установлен if PHP files are detected in portal directory)
...
# Check if nginx is установлен
if ! command -v nginx >/dev/null 2>&1; then
    LOG yellow "[WARNING] nginx не установлен."
    ...
    if [ "$resp" = "$DUCKYSCRIPT_USER_CONFIRMED" ]; then
        ...
        LOG "nginx установлен successfully"
    else
        ...
        *)
            if [ "$resp" = "$DUCKYSCRIPT_USER_CONFIRMED" ]; then
                ...
                for pkg in $GIT_PACKAGES; do
                    if ! opkg list-установлен | grep -q "^${pkg} "; then
                        LOG "    Installing $pkg..."
                        opkg install "$pkg"
                        if ! opkg list-установлен | grep -q "^${pkg} "; then
                        ...
                        LOG "    $pkg already установлен"
                    fi
                done
                
                if ! command -v git >/dev/null 2>&1; then
                    LOG red "  [ERROR] git installation failed"
                    ERROR_DIALOG "git installation не удалась! Cannot clone Evil Portals."
                else
                    LOG green "  git установлен successfully"
                fi
                ...
                if command -v git >/dev/null 2>&1; then
                    ...
                    if git clone https://github.com/kleo/evilportals.git "$CLONE_DIR" 2>&1 | while read line; do LOG "    $line"; done; then
                        ...
                        if [ -d "$CLONE_DIR/portals" ]; then
                            ...
                            LOG red "  [ERROR] portals directory не найден in clone"
                            rm -rf "$CLONE_DIR"
                            ERROR_DIALOG "portals directory не найден in cloned repository!"
                        fi
                    else
...
if [ "$SELECTED_DIR" = "default" ]; then
    ...
    if [ -f "$PAYLOAD_DIR/default_portal.html" ]; then
        ...
        LOG red "  [ERROR]: default_portal.html не найден in payload directory"
        exit 1
    fi
fi
...
if [ -n "$PHP_FILES" ]; then
    ...
    # Check if php8-fpm is установлен
    if ! which php-fpm >/dev/null 2>&1 && ! ls /usr/bin/php-fpm* >/dev/null 2>&1 && ! opkg list-установлен | grep -q php8-fpm; then
        LOG yellow "[WARNING] php-fpm не установлен. PHP files will not work."
        ...
        case $? in
            ...
            *)
                if [ "$resp" = "$DUCKYSCRIPT_USER_CONFIRMED" ]; then
                    ...
                    if ! opkg list-установлен | grep -q php8-fpm; then
                        LOG red "[ERROR] PHP installation failed. PHP files will not work."
                        exit 1
                    else
                        LOG green "[Success] PHP установлен"
                    fi
                else
        ...
        LOG "PHP-FPM already установлен"
    fi
...
if [ "$PHP_REQUIRED" -eq 1 ]; then
    if [ -S /var/run/php8-fpm.sock ]; then
        ...
        FPM_SOCK="/var/run/php8-fpm.sock"
        LOG "  Warning: PHP-FPM socket не найден, using default: $FPM_SOCK"
    fi
    ...
    if [ -f "$PAYLOAD_DIR/captiveportal.php" ]; then
        ...
        LOG red "  [ERROR]: captiveportal.php не найден in payload directory"
        exit 1
    fi
fi
...
if [ "$PHP_REQUIRED" -eq 1 ]; then
    ...
    if ls /var/run/php*fpm*.sock >/dev/null 2>&1; then
        ...
        LOG yellow "  [WARNING]: PHP-FPM socket не найден!"
    fi
fi
...
if ! nginx -t 2>&1 | grep -q "test is successful"; then
    LOG red "[ERROR] nginx configuration test не удалась!"
...
if [ -f "$PAYLOAD_DIR/whitelist_monitor.sh" ]; then
    ...
    LOG red "  [ERROR]: whitelist_monitor.sh не найден in payload directory"
    exit 1
fi
