...
download_tailscale() {
    ...
    if [ ! -d "/root" ] || [ ! -w "/root" ]; then
        ...
        LOG red "ERROR: /root не present or not writable; cannot proceed without /root"
        exit 1
    fi
    ...
    if [ ! -f "$filename" ]; then
        ERROR_DIALOG "Downloaded file не найден"
    ...
    if ! nice -n -5 tar -xzf "$filename" -C "$TMP_DIR" \
        ...
        LOG red "ERROR: Ошибка распаковки $filename"
        LOG "tar output:"
...
install_binaries() {
    ...
    if [ ! -d "$extract_dir" ]; then
        ERROR_DIALOG "Extracted files не найден"
    ...
    LOG green "Найдено extracted directory: $extract_dir"

    # Check if binaries exist in extracted directory
    if [ ! -f "$extract_dir/tailscale" ]; then
        ERROR_DIALOG "tailscale binary не найден"
    ...
    if [ ! -f "$extract_dir/tailscaled" ]; then
        ERROR_DIALOG "tailscaled binary не найден"
    ...
    if ! touch "$install_test" 2>/dev/null; then
        ...
        LOG red "ERROR: $INSTALL_DIR не writable"
        cleanup
        exit 1
    fi
    ...
    if [ ! -w "$SYMLINK_DIR" ]; then
        ...
        LOG red "ERROR: $SYMLINK_DIR не writable"
        cleanup
        exit 1
    fi
    ...
    LOG green "Binaries установлен successfully"
...
main_install() {
    ...
    if check_установлен; then
        resp=$(CONFIRMATION_DIALOG "Tailscale already установлен. Reinstall?")
        case $? in
    ...
    LOG ""
    LOG green "✓ Tailscale binaries установлен"
