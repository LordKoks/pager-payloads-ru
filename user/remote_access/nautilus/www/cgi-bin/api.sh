...
delete_payload() {
    ...
    case "$payload_path" in
        *..*)
            echo "Content-Type: application/json"
            echo ""
            echo '{"error":"Безопасность: обход пути запрещён"}'
            exit 1
            ;;
    esac
...
run_payload() {
    ...
    if ! validate_token "$token"; then
        ...
        echo ""
        echo "Защита CSRF: недействительный или отсутствующий токен. Обновите страницу и попробуйте снова."
        exit 1
    fi
    ...
    case "$rpath" in
        *..*)
            echo "Content-Type: text/plain"
            echo ""
            echo "Безопасность: обход пути запрещён"
            exit 1
            ;;
    esac
        ...
        if [ -n "$info" ]; then
            ...
            if echo "$essid_line" | grep -q '"'; then
                ...
                if [ -n "$ssid" ]; then
                    ...
                    mode=$(echo "$info" | grep "Режим:" | sed 's/.*Режим: \([^ ]*\).*/\1/')
                fi
            fi
        fi
    fi
    ...
    while IFS= read -r line; do
        case "$line" in
            ...
            *"Channel: "*)
                ...
                current_mode=$(echo "$line" | sed 's/.*Режим: \([^ ]*\).*/\1/')
                ;;
            *"Signal: "*)
...
install_github() {
    ...
    case "$github_url" in
        ...
        *)
            echo "Content-Type: text/plain"
            echo ""
            echo "Безопасность: разрешены только репозитории wifipineapplepager-payloads, wifipineapplepager-themes и wifipineapplepager-ringtones"
            exit 1
            ;;
    esac
    ...
    if [ "$install_type" = "ringtone" ]; then
        ...
        if [ "$dl_ok" = "1" ]; then
            sse_msg "green" "[Install] Successfully установлен $payload_name!"
    ...
    if [ "$install_type" = "theme" ]; then
        ...
        if command -v git >/dev/null 2>&1; then
            ...
            sse_msg "red" "[Install] Git не найден on device"
            rm -rf "$INSTALL_DIR"
    ...
    if [ ! -f "$INSTALL_DIR/$main_file" ]; then
        sse_msg "red" "[Install] $main_file не найден in downloaded files"
        printf 'event: done\ndata: {"status":"error","message":"Main file не найден"}\n\n'
        rm -rf "$INSTALL_DIR"
        rm -f /tmp/nautilus_install_target
        exit 1
    fi
...
install_pr() {
    ...
    if [ "$install_type" = "ringtone" ]; then
        ...
        if [ "$dl_ok" = "1" ]; then
            sse_msg "green" "[Install] Successfully установлен $payload_name from PR!"
            sse_msg "cyan" "[Install] Location: $target_path"
            rm -rf "$INSTALL_DIR"
            rm -f /tmp/nautilus_install_target
            /root/payloads/user/remote_access/nautilus/build_cache.sh >/dev/null 2>&1
...
run_github() {
    ...
    case "$github_url" in
        ...
        *)
            echo "Content-Type: text/plain"
            echo ""
            echo "Безопасность: разрешены только репозитории: wifipineapplepager-payloads, wifipineapplepager-ringtones"
            exit 1
            ;;
    esac
    ...
    if [ ! -f "$GITHUB_TARGET" ]; then
        sse_msg "red" "[GitHub] $target_type file не найден in downloaded files"
        printf 'event: done\ndata: {"status":"error"}\n\n'
        rm -rf "$GITHUB_DIR"
        exit 1
    fi
...
download_loot_all() {
    ...
    if [ ! -d "$loot_dir" ]; then
        echo "Content-Type: application/json"
        echo ""
        echo '{"error":"Loot directory не найден"}'
        return
    fi
