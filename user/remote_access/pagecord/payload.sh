...
Authenticate() {
    ...
    if [[ "$command_result" == *"$password"* ]]; then
        ...
        json_payload="{
          ...
          \"embeds\": [
            {
              \"title\": \":pager:   **PageCord Session Подключено!**   :pager:\",
...
download_attachment() {
    ...
    if [ -n "$attachment_url" ]; then
        ...
        echo "Вложение не найдено или неверная команда для скачивания."
    fi
}
        ...
        if [[ "$command" == "payloads"* ]]; then
        ...
        
            {
                ...
                if [[ ! -d "$payload_root" ]]; then
                    echo "[ERROR] Directory не найден: $payload_root"
                else
                    shopt -s nullglob
...
json_payload="{
  ...
  \"embeds\": [
    ...
    case "$bgresp" in
        $DUCKYSCRIPT_USER_CONFIRMED)
            LOG green "Фоновая сессия запускается.."
            echo "Фоновая сессия запускается.."
            Background_Session
            return
            ;;
