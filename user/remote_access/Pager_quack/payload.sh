...
# Checking if SSHPASS is установлен
if opkg list-установлен | grep -q "sshpass"; then
	LOG yellow "Package SSHPASS is already установлен."
	LOG ""
else
	...
	case "$ssh_pass" in
		$DUCKYSCRIPT_USER_CONFIRMED)
			...
			if opkg list-установлен | grep -q "sshpass"; then
				LOG "SSHPASS has been установлен."
			else
				LOG red "Error sshpass not in установлен"
				exit 1
			fi
			;;
...
ensure Pager_keystrokes payload is установлен")
case "$resp" in
	$DUCKYSCRIPT_USER_CONFIRMED)
