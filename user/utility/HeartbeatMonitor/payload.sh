...
ХРАНИЛИЩЕ_WARN=$(NUMBER_PICKER "Использование хранилища warn (%):" 90)
...
Interval: ${INTERVAL}s
Длительность: ${DURATION}h
Temp warn: ${TEMP_WARN}C
Temp crit: ${TEMP_CRIT}C
Mem warn: ${MEM_WARN}%
Хранилище warn: ${ХРАНИЛИЩЕ_WARN}%
...
echo "[$(date)] Thresholds: temp_warn=${TEMP_WARN}C temp_crit=${TEMP_CRIT}C mem_warn=${MEM_WARN}% storage_warn=${ХРАНИЛИЩЕ_WARN}%" >> "$ALERT_LOG"
...
while [ $(date +%s) -lt $END_TIME ]; do
    ...
    if [ $MEM_PCT -ge $MEM_WARN ]; then
        ...
        echo "[$NOW] WARNING: Использование памяти ${MEM_PCT}% (${MEM_USED}/${MEM_TOTAL}MB)" >> "$ALERT_LOG"
    fi
    ...
    if [ $ХРАНИЛИЩЕ_ROOT -ge $ХРАНИЛИЩЕ_WARN ]; then
        ALERT_MSG="${ALERT_MSG}ROOT: ${ХРАНИЛИЩЕ_ROOT}% "
        echo "[$NOW] WARNING: Root storage ${ХРАНИЛИЩЕ_ROOT}% full" >> "$ALERT_LOG"
    fi
    if [ $ХРАНИЛИЩЕ_MMC -ge $ХРАНИЛИЩЕ_WARN ]; then
        ALERT_MSG="${ALERT_MSG}SD: ${ХРАНИЛИЩЕ_MMC}% "
        echo "[$NOW] WARNING: SD card ${ХРАНИЛИЩЕ_MMC}% full" >> "$ALERT_LOG"
    fi
...
Использование памяти:    ${MEM_PEAK}% (warn: ${MEM_WARN}%)
