...
node_desc () {
  case "$1" in
    ...
    "Консерватория") say "Broken glass. Vines twist." ;;
...
move_node () {
  ...
  case "$cur,$d" in
    ...
    "East Hall,n") echo "Консерватория" ;;
    "East Hall,e") echo "Clown Den" ;;
    "Study,s") echo "North Hall" ;;
    "Консерватория,s") echo "East Hall" ;;
...
scene_haunted_map () {
  ...
  while true; do
    ...
    case "$current_node" in
      ...
      "Консерватория") haunted_trigger "Консерватория" ;;
