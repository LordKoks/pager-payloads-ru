...
EOF
    ...
    if [ $curl_exit -ne 0 ]; then
        LOG red "ОШИБКА: Ошибка сетевого запроса (curl exit: $curl_exit)"
        return 1
    fi
    ...
    if [ "$http_code" != "200" ]; then
        LOG red "ОШИБКА: API вернул HTTP $http_code"
    ...
    if [ -z "$game_json" ]; then
        LOG red "ОШИБКА: Не удалось извлечь содержимое игры from response"
        return 1
    fi
    ...
    if ! echo "$game_json" | jq -e '.nodes.start.choice_up and .nodes.start.choice_down' >/dev/null 2>&1; then
        LOG red "ОШИБКА: Сгенерированная игра имеет неверную структуру"
...
get_choice() {
    ...
    while true; do
        ...
        if [ -z "$next_node" ]; then
            LOG red "ОШИБКА: Неверное состояние игры"
        ...
        if ! jq -e ".nodes[\"$next_node\"]" "$GAME_FILE" >/dev/null 2>&1; then
            LOG red "ОШИБКА: Целевой узел '$next_node' не найден"
            return 1
        fi
...
if ! command -v jq >/dev/null 2>&1; then
    ERROR_DIALOG "jq не установлен"
    LOG red "FATAL: Missing codec - jq не найден"
...
if [ -z "$ANTHROPIC_API_KEY" ]; then
    ERROR_DIALOG "Нет учётных данных Metaverse"
...
if ! check_network; then
    ERROR_DIALOG "Восходящий канал не обнаружен"
...
if ! generate_game; then
    ERROR_DIALOG "Ошибка генерации игры"
    exit 1
fi
