...
if [ -z "$IFACE" ]; then
    ERROR_DIALOG "Интерфейс WiFi не найден!

Убедитесь, что адаптер WiFi
is connected."
    exit 1
fi
...
echo "" >> "$REPORT"
echo "Самые целевые устройства:" >> "$REPORT"
