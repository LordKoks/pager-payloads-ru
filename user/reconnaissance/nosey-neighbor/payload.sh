...
if [ -z "$MON_IFACE" ]; then
    ...
    if [ -z "$MON_IFACE" ]; then
        for IFACE in $(ls /sys/class/net/ 2>/dev/null); do
            MODE=$(iwinfo "$IFACE" info 2>/dev/null | grep -i "Режим:" | awk '{print $NF}')
    ...
    if [ -z "$MON_IFACE" ]; then
        for IFACE in wlan0mon wlan1mon mon0 mon1 phy0-mon0 phy1-mon0; do
            if [ -d "/sys/class/net/$IFACE" ]; then
                MON_IFACE="$IFACE"
                printf "[DEBUG] Найдено monitor iface by name: %s\n" "$MON_IFACE" >> "$DEBUG_LOG"
                break
            fi
        done
    fi
...
if [ -z "$SCAN_IFACE" ]; then
    for IFACE in $(ls /sys/class/net/ 2>/dev/null); do
        MODE=$(iwinfo "$IFACE" info 2>/dev/null | grep -i "Режим:")
        if echo "$MODE" | grep -qi "Master\|Client\|managed"; then
            SCAN_IFACE="$IFACE"
            printf "[DEBUG] Найдено managed iface: %s (%s)\n" "$IFACE" "$MODE" >> "$DEBUG_LOG"
            break
        fi
    done
fi
...
if [ "$AP_COUNT" -gt 0 ]; then
    ...
    if [ -f "$AP_DB" ]; then
        ...
        END {
            ...
            printf "  │  Unique Named Сетьs:   %d\n", idx
            printf "  │  Hidden Сетьs:         %d\n", hidden_count
            printf "  │  ISP Hotspots:            %d\n", isp_count
            printf "  │  Private Сетьs:        %d\n", private_count
            printf "  │  Est. Physical Devices:   ~%d\n", phys
            printf "  │\n"

            if (private_count > 0) {
                printf "  │  PRIVATE СЕТЬS\n"
...
if [ "$AP_COUNT" -gt 0 ] && [ -f "$AP_DB" ]; then
    ...
    END {
        ...
        printf "  │    - Guest networks (e.g. MyNet_Guest alongside MyСеть)\n"
...
if [ "$ENABLE_PCAP_SNAPSHOT" = "true" ]; then
    LED R SOLID
    LOG "Nosey Neighbor: Запуск захвата пакетов на ${MON_IFACE}"
    ...
    if [ -f "$PCAP_FILE" ] && [ -s "$PCAP_FILE" ]; then
        ...
        printf "  │    wlan.ssid == \"TargetСетьName\"\n" >> "$REPORT_FILE"
        printf "  │\n" >> "$REPORT_FILE"
...
if [ -f "$AP_DB" ]; then
    ...
    if [ -n "$WEP_NETS" ]; then
        ...
        printf "  [!] WEP СЕТЬS DETECTED: %d (trivially crackable)\n" "$WEP_COUNT" >> "$REPORT_FILE"
...
if [ "$ENABLE_VENDOR_LOOKUP" = "true" ] && [ -f "$VENDOR_FILE" ] && [ -s "$VENDOR_FILE" ]; then
    ...
    END {
        ...
        if (infra)    printf "    Сеть infra:      %d device(s)\n", infra
