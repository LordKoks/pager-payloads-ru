...
install_tools() {
    ...
    if check_tools; then
        LOG "Tools установлен successfully"
        return 0
    else
...
if ! check_tools; then
    LOG "Required tools не установлен"
...
if [ "$(CONFIRMATION_DIALOG "SCAN ALL СЕТЬS?\n\nYes = All APs at once\nNo = Pick one target")" = "1" ]; then
