...
# Net Режим: TRANSPARENT
# Dependencies: airmon-ng, tcpdump, iw, md5sum (all pre-установлен on Pager)
#
...
# If stdin не a TTY (running headlessly from Pager UI),
...
try_install() {
    ...
    if opkg install "$PKG" 2>&1 | tee -a "$LOG_FILE" | grep -q "Installed\|up to date"; then
        log_msg "OK: $PKG успешно установлен."
        return 0
    else
...
check_deps() {
    ...
    if ! iw dev "$BASE_IFACE" info >/dev/null 2>&1; then
        log_msg "ERROR: Interface '$BASE_IFACE' не найден and MONITOR_IFACE not in monitor mode."
        log_msg "Available Интерфейсы:"
...
#     Device probes for an SSID in the enrolled Preferred Сеть List.
...
if [ -z "$TARGET_MAC_CLEAN" ]; then
    ...
    log_msg "ERROR: TARGET_MAC '$TARGET_MAC' не a valid MAC address."
    exit 1
fi
