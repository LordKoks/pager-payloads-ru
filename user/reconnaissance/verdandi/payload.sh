...
#     - Специфичное для производителя IEs - Tag 221
...
#           - Added Специфичное для производителя (Tag 221) extraction
...
start_probe_capture() {
    ...
    # -s 1024: Capture enough for all IEs including Специфичное для производителя
    # -l: Line buffered
...
#   Tag 221: Специфичное для производителя (OUI + data)
...
extract_fingerprint() {
    ...
    # Специфичное для производителя: Tag 221 (0xdd)
...
analyze_fingerprints() {
    ...
    {
        echo "=============================================="
        echo "  VERDANDI v2.0  ОТЧЁТ ОБ ОТПЕЧАТКАХ ПРОБ"
        echo "  Generated: $(date)"
        echo "=============================================="
        echo ""
        echo "SUMMARY"
        ...
        while IFS='|' read -r fingerprint macs_list; do
            ...
            if [ -n "$ssids_line" ]; then
                echo "Сетьs Probed: $ssids_line"
            else
                echo "Пробные сети: (только широковещательные  режим конфиденциальности)"
            fi
        ...
        if [ "$randomization_detected" -gt 0 ]; then
            echo "VERDANDI отследила эти устройства по смене MAC, используя:"
            echo "  - Порядок IE-тегов (сигнатура драйвера)"
            echo "  - Возможности HT/VHT/HE (радио-характеристики)"
            echo "  - Расширенные возможности (отпечаток ОС)"
            echo "  - Vendor OUI (подсказки производителя)"
            echo "  - RSSI (различие по расстоянию)"
            echo ""
        fi

        # Show any fingerprints with significantly different RSSI
        echo "=============================================="
        echo "  КОГОРТНЫЙ АНАЛИЗ"
        echo "=============================================="
        echo ""
        ...
        while IFS='|' read -r fp rssi_range; do
            ...
            if [ "$diff" -gt 20 ] 2>/dev/null; then
                ...
                echo "  Возможно несколько людей с одинаковой моделью устройства"
                echo ""
            fi
        done < "$FP_TO_RSSI"
        ...
        echo "=============================================="
        echo ""
        echo "База отпечатков:"
        cat "$FINGERPRINT_DB" 2>/dev/null
        echo ""
        echo "Соответствия SSID:"
...
if ! iw dev "$WIFI_INTERFACE" info >/dev/null 2>&1; then
    if iw dev "wlan0mon" info >/dev/null 2>&1; then
        ...
        ERROR_DIALOG "Monitor interface не найден!"
        exit 1
    fi
fi
