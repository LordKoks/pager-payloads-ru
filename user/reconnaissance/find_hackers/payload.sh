...
search_ssid_json() {
    ...
    if [[ -z "$search_ssid" ]]; then
        ...
        if [[ "$mode" == "SEARCH" ]]; then
            ...
            echo "Недопустимый режим. Используйте SEARCH, ISEARCH, SUBSTRING или ISUBSTRING"
            return 1
        fi
...
alert_aps_spoofing_ssids() {
    ...
    log_to_file "Найдено $spoof_count APs spoofing atleast $MIN_SPOOFING_COUNT networks"
	LOG "Найдено $spoof_count APs spoofing atleast $MIN_SPOOFING_COUNT networks\n"
}
...
# Returns 0 if encryption is OPN. Returns 1 if encryption не OPN.
is_open_network() {
	local mac="$1"
	local enc="$2"
...
alert_evil_twin() {
	...
	log_to_file "Найдено $evil_count potential evil twins"
	LOG "Найдено $evil_count potential evil twins\n"
}
...
alert_flipper_bt() {
	...
	log_to_file "Найдено ${#BT_FLIPPERS[@]} potential Flipper Zero devices"
    LOG "Найдено ${#BT_FLIPPERS[@]} potential Flipper Zero devices"
}
...
find_pagers() {
    ...
    log_to_file "Найдено ${#AP_RESULTS[@]} APs with substring 'pager'"
    LOG "Найдено ${#AP_RESULTS[@]} APs with substring 'pager'\n"
    alert_sus_aps
}
...
find_pineapples() {
    ...
    log_to_file "Найдено ${#AP_RESULTS[@]} APs with substring 'pineapple'"
    LOG "Найдено ${#AP_RESULTS[@]} APs with substring 'pineapple'\n"
    alert_sus_aps
}
...
find_rayhunters() {
    ...
    log_to_file "Найдено ${#AP_RESULTS[@]} APs with substring 'RC400L'"
    LOG "Найдено ${#AP_RESULTS[@]} APs with substring 'RC400L'\n"
    alert_sus_aps
}
