...
if ! command -v curl >/dev/null 2>&1; then
    LOG "curl не установлен. Installing via opkg..."
    opkg update
    opkg install curl
...
if ! command -v jq >/dev/null 2>&1; then
    LOG "jq не установлен. Installing via opkg..."
    opkg update
    opkg install jq
