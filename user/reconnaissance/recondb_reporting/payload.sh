...
if ! which sqlite3 > /dev/null 2>&1; then
    ALERT "sqlite3 не найден. Run: opkg install sqlite3-cli"
    exit 1
fi

if [ ! -f $RECON_DB ]; then
    ALERT "Recon database не найден at $RECON_DB"
    exit 1
fi
...
echo "================================================================================"
echo "  ОТЧЁТ О БЕСПРОВОДНОЙ РАЗВЕДКЕ"
echo "  WiFi Pineapple Pager  Hak5 (8-е поколение PineAP)"
echo "================================================================================"
echo ""
...
echo "================================================================================"
echo ""
echo "  BSSID               SSID                     Диапазон  Кан  Шифрование          Сигн  Пакеты  Последний раз"
echo "  ----------------------------------------------------------------------------------------------------"
...
echo ""
echo "================================================================================"
echo "  SECTION 3 -- OPEN СЕТЬS  ($ap_open found)"
echo "================================================================================"
if [ "$ap_open" -eq 0 ]; then
    echo "  Открытых точек доступа не наблюдалось."
else
    echo "  !! Без шифрования  трафик клиента виден в открытом виде."
    echo ""
...
echo ""
echo "  MAC                  Пробный SSID              Сигн  Диапазон  Первый раз"
echo "  ------------------------------------------------------------------------"
if [ "$client_total" -eq 0 ]; then
    echo "  Клиентские устройства не наблюдались."
else
...
echo ""
echo "================================================================================"
echo "  РАЗДЕЛ 5  ЗАХВАТЫ HANDSHAKE"
echo "================================================================================"
if [ "$hs_pcap" -eq 0 ]; then
    echo "  Handshake не захвачены."
...
echo ""
echo "================================================================================"
echo "  РАЗДЕЛ 6  ПЕРЕХВАЧЕННЫЕ УЧЁТНЫЕ ДАННЫЕ (Злая WPA)"
echo "================================================================================"
if [ "$cred_count" -eq 0 ]; then
    echo "  Учётные данные не захвачены."
else
...
echo ""
echo "================================================================================"
echo "  РАЗДЕЛ 7  GPS И МЕСТОПОЛОЖЕНИЯ ТОЧЕК ДОСТУПА"
echo "================================================================================"
if [ $gps_available -eq 1 ]; then
    echo "  Фиксация GPS на момент отчёта:"
    ...
    echo "  Нет фиксации GPS. Подключите USB GPS и обеспечьте открытое небо."
fi
echo ""
...
echo ""
echo "================================================================================"
echo "  РАЗДЕЛ 8  ОБНАРУЖЕНИЯ ЛОЖНЫХ ТОЧЕК ДОСТУПА"
echo "================================================================================"
if [ "$rogue_count" -eq 0 ]; then
    if [ -f $ROGUE_LOG ]; then
        echo "  Детектор отработал  событий не обнаружено."
    else
        echo "  Журнал обнаружения отсутствует. Запустите сценарий Rogue AP Detector для включения."
    fi
else
...
echo ""
echo "================================================================================"
echo "  РАЗДЕЛ 9  СБОР ДОБЫЧИ"
echo "================================================================================"
echo ""
echo "  IP Пейджера: 172.16.42.1"
echo ""
