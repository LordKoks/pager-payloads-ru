...
send_to_discord() {
    ...
    if [ ! -f "$LOOTFILE" ]; then
        LOG "Loot file не найден: $LOOTFILE"
        return
    fi
...
scan_parameters() {
    ...
    if [ ${#found_params[@]} -gt 0 ]; then
        log_result ""
        log_result "[*] SUMMARY: Найдено ${#found_params[@]} interesting parameter(s):"
...
scan_cookies() {
    ...
    if [ -z "$cookies" ]; then
        ...
        log_result "[*] Найдено $cookie_count cookie(s) in headers"
    ...
    if [ "$js_cookie_sets" -gt 0 ]; then
        js_cookies_found=1
        log_finding "INFO" "Найдено $js_cookie_sets JavaScript cookie assignment(s) (document.cookie=)"
    ...
    if [ "$js_cookie_reads" -gt 0 ]; then
        log_result "[*] Найдено $js_cookie_reads JavaScript cookie read(s) (document.cookie)"
    ...
    if [ -z "$cookies" ] && [ $js_cookies_found -eq 0 ]; then
        if [ $consent_detected -eq 1 ]; then
            ...
            log_result ""
            log_result "Or use DevTools Сеть tab to see Set-Cookie headers"
            ...
            log_result "3. Application tab → Хранилище → Cookies"
            log_result ""
...
scan_wordpress_vulns() {
    ...
    if [ -n "$rss_version" ]; then
        ...
        LOG "Найдено WP version $wp_version via RSS feed"
    fi
    ...
    if [ -n "$atom_version" ]; then
        ...
        LOG "Найдено WP version $atom_version via Atom feed"
    fi
    ...
    if [ -n "$meta_version" ]; then
        ...
        LOG "Найдено WP version $meta_version via meta tag"
    fi
    ...
    if [ -n "$opml_version" ]; then
        ...
        LOG "Найдено WP version $opml_version via OPML"
    fi
    ...
    if [ -n "$readme_version" ]; then
        ...
        LOG "Найдено WP version $readme_version via readme.html"
    fi
    ...
    if [ -z "$wp_version" ]; then
        ...
        if [ -n "$asset_version" ]; then
            ...
            LOG "Найдено likely WP version $asset_version via core asset versions"
        fi
    fi
    ...
    log_result " | Найдено By: $wp_detected_by"
    ...
    if [ "$vuln_count" -eq 0 ] || [ -z "$vuln_count" ]; then
        ...
        if [ "$base_api_version" != "$api_version" ]; then
            ...
            if [ -n "$api_response" ] && ! echo "$api_response" | grep -qiE '"error"'; then
                ...
                LOG "Найдено $vuln_count vulnerabilities under base version ${major}.${minor}"
            fi
        fi
    fi
        ...
        if [ "$provider" = "aws" ]; then
            for kw in 'ami-id' 'instance-id' 'instance-type' 'local-ipv4' 'public-hostname' 'security-credentials'; do
                if echo "$response" | grep -q "$kw" && ! echo "$baseline" | grep -q "$kw"; then
                    return 0  # Найдено metadata not in baseline
                fi
            done
        fi
...
scan_crt_sh() {
    ...
    local PROBE_CAP=25
    log_result "[*] Найдено $count unique subdomain(s) in CT logs:"
