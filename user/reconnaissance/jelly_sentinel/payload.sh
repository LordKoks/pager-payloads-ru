...
phase_authorization() {
    ...
    local target
    target=$(TEXT_PICKER "Сеть / client name" "HomeСеть")
    case $? in "$DUCKYSCRIPT_CANCELLED"|"$DUCKYSCRIPT_REJECTED"|"$DUCKYSCRIPT_ERROR")
        LOG red "Cancelled"; exit 0 ;; esac
    ...
    {
        echo "================================================"
        echo "         ОЦЕНКА БЕЗОПАСНОСТИ JELLY SENTINEL"
        echo "================================================"
        echo "Tester   : $TESTER"
        echo "Target   : $TARGET"
        echo "Date     : $(date)"
        echo "Scan Режим: $SCAN_MODE"
        echo "GPS      : $GPS_COORDS"
        echo "Согласие: ПОДТВЕРЖДЕНО"
        echo "Инструмент: Jelly Sentinel v1.0 / WiFi Pineapple Pager"
        echo "================================================"
        echo ""
    } > "$REPORT"

    echo "Критичность,Название,Описание,CVSS,CVE,Исправление" > "$CSV_FILE"
    LOG green "Auth logged | Режим: $SCAN_MODE"
    sleep 1
}
...
phase_wifi_audit() {
    ...
    { echo "--- ФАЗА 1: WiFi-АУДИТ ---"; echo ""; } >> "$REPORT"
    ...
    if [ "$SCAN_MODE" != "STEALTH" ]; then
        ...
        if [ -f "$RECON_DB" ]; then
            ...
            [ "${open_count:-0}" -gt 0 ] && \
                add_finding "HIGH" "Open WiFi Сетьs ($open_count)" \
...
phase_discovery() {
    ...
    { echo "--- ФАЗА 2: ОБНАРУЖЕНИЕ УСТРОЙСТВ ---"; echo ""; } >> "$REPORT"
...
phase_fingerprint() {
    ...
    { echo "--- ФАЗА 3: СНЯТИЕ ОТПЕЧАТКОВ ---"; echo ""
    ...
    if [ -s "$BANNER_CVE_FILE" ]; then
        { echo "--- СОВПАДЕНИЯ CVE-БАННЕРОВ ---"; echo ""; } >> "$REPORT"
...
phase_risk_checks() {
    ...
    { echo "--- ФАЗА 4: ПРОВЕРКИ РИСКОВ ---"; echo ""; } >> "$REPORT"
...
phase_bluetooth() {
    ...
    { echo "--- ФАЗА 5: BLUETOOTH-СКАНИРОВАНИЕ ---"; echo ""; } >> "$REPORT"
    ...
    if [ "${bt_count:-0}" -gt 0 ]; then
        ...
        echo "Bluetooth-устройства не найдены" >> "$BT_FILE"
        echo "  Bluetooth-устройства не найдены" >> "$REPORT"
    fi
...
phase_traffic_analysis() {
    ...
    { echo "--- ФАЗА 6: ПАССИВНЫЙ АНАЛИЗ ТРАФИКА ---"; echo ""; } >> "$REPORT"
...

    {
        ...
        echo ""
        echo "  Самые активные участники:"
        head -8 "$TALKERS_FILE" | while read -r cnt pair; do
            printf "    %-6s %s\n" "$cnt" "$pair"
        done
        echo ""
        echo "  Топ DNS-запросов:"
...
phase_delta() {
    ...
    { echo "--- ИЗМЕНЕНИЯ ПО СРАВНЕНИЮ С ПРЕДЫДУЩИМ СКАНИРОВАНИЕМ ---"
...
phase_executive_summary() {
...

    {
        ...
        echo "================================================"
        echo ""
        printf "  Сеть Security Score : %d / 100\n" "$RISK_SCORE"
        printf "  Risk Level             : %s\n"        "$risk_label"
...
phase_report() {
    ...
    {
        echo "================================================"
        echo "Сгенерировано Jelly Sentinel v1.0 / WiFi Pineapple Pager"
...
LOG "Режим: $SCAN_MODE | Длительность: ${TRAFFIC_DURATION}s | Интерфейс: $IFACE"
