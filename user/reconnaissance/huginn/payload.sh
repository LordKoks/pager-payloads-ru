...
correlate_signals() {
    ...
    {
        echo "=============================================="
        echo "  ОТЧЁТ О КОРРЕЛЯЦИИ ЛИЧНОСТЕЙ HUGINN"
        echo "  Generated: $(date)"
        echo "=============================================="
        echo ""
        ...
        echo ""
        echo "Параметры корреляции:"
        echo "  Time Window:    ${TIME_WINDOW}s"
        echo "  Min Confidence: ${MIN_CONFIDENCE}%"
        echo ""
        echo "=============================================="
        echo "  КОРРЕЛИРОВАННЫЕ ЛИЧНОСТИ"
        echo "=============================================="
        echo ""
    } > "$CORRELATION_RESULTS"
    ...
    # Summary
    {
        ...
        if [ "$matches" -eq 0 ]; then
            echo "Совпадений с высокой уверенностью не найдено."
            echo "Это может означать:"
            echo "  - Устройства используют разных производителей для WiFi/BLE"
            echo "  - Сигналы не были захвачены в одном временном окне"
            echo "  - Рандомизация MAC скрывает личности"
        fi
        echo ""
        echo "=============================================="
        echo "  СЫРЫЕ СПИСКИ УСТРОЙСТВ"
        echo "=============================================="
        echo ""
        echo "--- WiFi-УСТРОЙСТВА ---"
...
if ! iw dev "$WIFI_INTERFACE" info >/dev/null 2>&1; then
    ERROR_DIALOG "WiFi interface $WIFI_INTERFACE не найден!
...
if ! hciconfig "$BLE_ADAPTER" >/dev/null 2>&1; then
    ERROR_DIALOG "BLE adapter $BLE_ADAPTER не найден!"
    exit 1
fi
