...
# Operation
ProbeHound uses tcpdump to sniff for probe request and then filters the requests based off of a target SSID. It is important to note that the tool specifically uses the network name and NOT the MAC address or BSSID. SSIDs can be selected from the PineAP SSID Pool, or manually entered. Manual entry accepts partial names, though the more complete the name, the more accurate the search will be.
