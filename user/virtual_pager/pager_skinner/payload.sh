...
SKINS_DIR="/mmc/root/ui_skins"                # Where установлен skins live
...
if ! command -v unzip >/dev/null 2>&1; then
    LOG red "unzip не найден. Installing..."
    opkg update
    opkg install unzip
    if [ $? -ne 0 ]; then
        LOG red "Ошибка установки unzip. Aborting."
        exit 1
    fi
fi
...
if [ -d "$SKINS_ZIP_DIR" ]; then
    for zipfile in "$SKINS_ZIP_DIR"/*.zip; do
        ...
        if [ ! -d "$target_dir" ] || [ -z "$(ls -A "$target_dir" 2>/dev/null)" ]; then
            ...
            if [ $? -eq 0 ]; then
                LOG green "Скин '$skin_name' успешно установлен."
                ((установлен_count++))
            else
                LOG red "Ошибка распаковки $zipfile"
            fi
        fi
    done
fi

if [ $установлен_count -gt 0 ]; then
    LOG green "$установлен_count new skin(s) установлен."
fi
...
LOG green "Скин '$CHOSEN_SKIN' успешно применён!"
