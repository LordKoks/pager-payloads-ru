...
echo "Здесь не на что смотреть :)" > "$WWW_ABS_PATH/index.html"
...
install_dependencies() {
    ...
    if ! command -v uhttpd >/dev/null 2>&1; then
        ...
        opkg install uhttpd || { LOG "Ошибка установки uhttpd"; exit 1; }
    fi
}
...
EOF
    ...
    LOG "Payload внедрён и сервер запущен на порту $SERVER_PORT."
}
...
remove_payload() {
    ...
    LOG "Payload удалён и сервер остановлен."
}
