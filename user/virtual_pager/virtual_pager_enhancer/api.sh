...
update_payloads(){
    ...
    if ! command -v unzip >/dev/null 2>&1; then
        ...
        opkg install unzip || { 
            ...
            echo ""
            echo '{"okay":false,"error":"Ошибка установки Unzip"}'
            exit 0; 
        }
    fi
    ...
    if ! wget -q --no-check-certificate "$PAYLOADS_URL" -O "$TEMP_DIR/master.zip"; then
        echo "Content-Type: application/json"
        echo ""
        echo '{"okay":false,"error":"Ошибка загрузки Payloads"}'
        exit 0
    fi
    ...
    if [ -d "$PAYLOAD_DIR" ]; then
        ...
        echo "Content-Type: application/json"
        echo ""
        echo '{"okay":true,"message":"Успешно обновлено"}'
    else
        echo "Content-Type: application/json"
        echo ""
        echo '{"okay":false,"error":"Не удалось распаковать Payloads"}'
    fi
