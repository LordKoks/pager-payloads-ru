...
# ====================================================================
LOG "Step 0: Verifying Evil Portal is установлен..."

if [ ! -f "/etc/init.d/evilportal" ]; then
    LOG "ERROR: Evil Portal не установлен"
    LOG "Please run the 'Install Evil Portal' payload first"
    exit 1
fi

LOG "SUCCESS: Evil Portal is установлен"
...
if [ ! -d "${PORTAL_DIR}" ]; then
    LOG "ERROR: Default portal не найден at ${PORTAL_DIR}"
    LOG "Please run the 'Install Evil Portal' payload first"
    exit 1
fi
