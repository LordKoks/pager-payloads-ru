...
LOG "SUCCESS: Сеть configuration updated"
...
LOG "SUCCESS: Сеть connectivity restored"
...
if [ "$TARGET_MODE" = "isolated" ]; then
    ...
    if uci show network | grep -q "name='br-evil'"; then
        ...
        LOG "ERROR: br-evil bridge не найден"
        exit 1
    fi
...
if [ "$TARGET_MODE" = "isolated" ]; then
    LOG "Bridge Интерфейс: ${BRIDGE_IF_EVIL}"
    LOG "Target Интерфейс: ${TARGET_IFACE}"
    LOG "Evil Portal now applies to ${TARGET_IFACE} only"
else
    LOG "Bridge Интерфейс: ${BRIDGE_IF_LAN}"
