...
LOG "Bridge Интерфейс: ${BRIDGE_IF}"
...
# Check which packages need to be установлен
PACKAGES_NEEDED=""

# Check PHP packages
if ! opkg list-установлен | grep -q "^php8 "; then
    PACKAGES_NEEDED="$PACKAGES_NEEDED php8"
fi
if ! opkg list-установлен | grep -q "^php8-fpm "; then
    PACKAGES_NEEDED="$PACKAGES_NEEDED php8-fpm"
fi
if ! opkg list-установлен | grep -q "^php8-mod-curl "; then
    PACKAGES_NEEDED="$PACKAGES_NEEDED php8-mod-curl"
fi
if ! opkg list-установлен | grep -q "^php8-mod-sqlite3 "; then
    PACKAGES_NEEDED="$PACKAGES_NEEDED php8-mod-sqlite3"
fi

# Check nginx packages
if ! opkg list-установлен | grep -q "^nginx-full "; then
    PACKAGES_NEEDED="$PACKAGES_NEEDED nginx-full"
fi
if ! opkg list-установлен | grep -q "^nginx-ssl-util "; then
    PACKAGES_NEEDED="$PACKAGES_NEEDED nginx-ssl-util"
fi
if ! opkg list-установлен | grep -q "^zoneinfo-core "; then
...
if [ -n "$PACKAGES_NEEDED" ]; then
    ...
    # Verify critical packages установлен successfully
    if ! opkg list-установлен | grep -q "php8-fpm"; then
    ...
    LOG "SUCCESS: All missing packages установлен"
else
    LOG "SUCCESS: All required packages already установлен (skipping installation)"
fi
...
switch() {
    ...
    if [ ! -d "\$PORTAL_BASE/\$NEW_PORTAL" ]; then
        ...
        logger -t evilportal "Switch failed: portal не найден (\$NEW_PORTAL)"
        return 1
    fi
    ...
    if [ \$? -ne 0 ]; then
        echo "Не удалось обновить символическую ссылку портала"
...
LOG "  Set Интерфейс: run set_evil_portal_interface payload"
LOG "=================================================="
