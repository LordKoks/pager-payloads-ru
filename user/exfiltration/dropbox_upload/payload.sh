...
load_token() {
    ...
    if [ ! -f "$TOKEN_FILE" ]; then
        LOG "ERROR" "Token file не найден"
        return 1
    fi
...
refresh_access_token() {
    ...
    if [ ! -f "$TOKEN_FILE" ]; then
        LOG "ERROR" "Cannot refresh: token file не найден"
        return 1
    fi
...
upload_directory() {
    ...
    for file in "$local_dir"/*; do
        if [ -f "$file" ]; then
            ...
            LOG "DEBUG" "Найдено file: $(basename "$file") ($fsize bytes)"
        fi
    done

    LOG "INFO" "Найдено $file_count files ($total_size bytes total)"
    ...
    LOG "INFO" "Длительность: ${duration}s, Total size: $total_size bytes"
...
0. Back to main menu"
    ...
    local choice
    choice=$(NUMBER_PICKER "Выберите опцию" 1)
...
main_upload() {
    ...
    if [ ! -e "$source" ]; then
        LOG "ERROR" "Source не найден: $source"
        ERROR_DIALOG "Source не найден"
        return 1
    fi
    ...
    # Log session
    {
        ...
        echo "  Длительность: ${session_duration}s"
...
$(if [ -f "$SESSION_LOG" ]; then
    ...
    echo "История загрузок отсутствует"
fi)
