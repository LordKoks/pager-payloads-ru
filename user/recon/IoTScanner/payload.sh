...
echo "Результаты IoT-сканера" > "$LOOT_FILE"
...
Format: 192.168.1.0/24"
    ...
    СЕТЬ=$(TEXT_PICKER "Сеть range:" "192.168.1.0/24")
    
    SPINNER_START "Active network scan..."
    
    # ARP scan
    ARP_RESULTS=$(arp-scan -I $INTERFACE $СЕТЬ 2>/dev/null || arping -c 1 $СЕТЬ 2>/dev/null)
