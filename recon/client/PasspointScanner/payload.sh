...
# Title: Сканер Passpoint
# Author: NullSec
# Description: Сканирование сетей Passpoint/Hotspot 2.0 и корпоративных конфигураций WiFi
...
PROMPT "СКАНЕР PASSPOINT

Обнаружение сетей Passpoint и
Hotspot 2.0.

Особенности:
- Обнаружение AP HS2.0
- Анализ RADIUS/EAP
- Перечисление корпоративного WiFi
- Консорциум роуминга
...
MONITOR_IF=""
for iface in wlan0mon wlan2mon mon0 wlan1 $IFACE; do
...
1. Быстрое сканирование Passpoint
2. Детальная информация HS2.0
3. Аудит корпоративного WiFi
4. Полное перечисление
...
DURATION=$(NUMBER_PICKER "Время сканирования (секунды):" 90)
...
BAND=$(CONFIRMATION_DIALOG "Включить 5GHz?
...
resp=$(CONFIRMATION_DIALOG "ЗАПУСТИТЬ СКАНИРОВАНИЕ PASSPOINT?
...
SPINNER_START "Сканирование AP HS2.0..."

echo "=======================================" > "$REPORT"
echo "    ОТЧЕТ СКАНИРОВАНИЯ NULLSEC PASSPOINT      " >> "$REPORT"
echo "=======================================" >> "$REPORT"
echo "" >> "$REPORT"
...
echo "--- КОРПОРАТИВНЫЕ СЕТИ WiFi ---" >> "$REPORT"
echo "" >> "$REPORT"
...
if [ "$SCAN_MODE" -ge 2 ] && [ -f "$PCAP_FILE" ]; then
    echo "--- ДЕТАЛИ HOTSPOT 2.0 ---" >> "$REPORT"
    echo "" >> "$REPORT"
...
if [ "$SCAN_MODE" -ge 3 ]; then
    echo "--- КОРПОРАТИВНЫЙ АУДИТ ---" >> "$REPORT"
    echo "" >> "$REPORT"

    echo "Наблюдаемые типы EAP:" >> "$REPORT"
    if command -v tshark >/dev/null 2>&1 && [ -f "${CAP_PREFIX}"*.cap ] 2>/dev/null; then
        ...
        echo "  (для анализа EAP требуется tshark)" >> "$REPORT"
    fi

    echo "" >> "$REPORT"
    echo "Сводка корпоративной безопасности:" >> "$REPORT"
    echo "  Всего корпоративных AP: $ENTERPRISE_COUNT" >> "$REPORT"
    echo "  Кандидатов Passpoint: $PASSPOINT_COUNT" >> "$REPORT"
    echo "  Всего AP просканировано: $TOTAL_APS" >> "$REPORT"
fi
...
PROMPT "СКАНИРОВАНИЕ PASSPOINT ЗАВЕРШЕНО

Всего AP: $TOTAL_APS
Корпоративных: $ENTERPRISE_COUNT
