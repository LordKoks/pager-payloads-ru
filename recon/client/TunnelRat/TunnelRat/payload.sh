...
if ! command -v ssh >/dev/null 2>&1; then
    ERROR_DIALOG "SSH client не найден!
...
$KEY_FILE"; exit 1
    ...
    [ -z "$SSH_PASS" ] && { ERROR_DIALOG "Требуется пароль!"; exit 1; }
    if ! command -v sshpass >/dev/null 2>&1; then
        ERROR_DIALOG "sshpass не установлен!
...
Auto-reconnect: ON
$([ $TUNNEL_MODE -eq 2 ] && echo 'SOCKS-прокси: порт 1080')
$([ $TUNNEL_MODE -eq 3 ] && echo 'Web forward: порт 8080')
