...
# Title: Сниффер Пакетов
# Author: NullSec
# Description: Продвинутый захват пакетов с фильтрами по протоколам и статистикой
...
PROMPT "СНИФФЕР ПАКЕТОВ

Продвинутый движок захвата
пакетов с учетом протоколов.
...
IFACE=""
for i in wlan1mon wlan2mon mon0 br-lan eth0 wlan1 $IFACE; do
    [ -d "/sys/class/net/$i" ] && IFACE=$i && break
done
[ -z "$IFACE" ] && { ERROR_DIALOG "Интерфейс захвата не найден!"; exit 1; }
...
3. Email (SMTP/IMAP)
4. DNS запросы
5. Все протоколы открытым текстом
6. Полный сырой захват
7. Кастомный BPF фильтр
