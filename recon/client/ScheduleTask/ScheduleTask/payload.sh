...
Press OK to exit."
            ...
            ERROR_DIALOG "Ошибка добавления задачи!
...
Press OK to browse."
        ...
        [ -z "$PAYLOADS" ] && { ERROR_DIALOG "Сценарии не найдены in /mmc/payloads/"; exit 1; }

        PAYLOAD_NAME=$(TEXT_PICKER "Payload dir:" "$(echo "$PAYLOADS" | head -1)")
        case $? in $DUCKYSCRIPT_CANCELLED|$DUCKYSCRIPT_REJECTED) exit 0 ;; esac

        PAYLOAD_PATH="/mmc/payloads/$PAYLOAD_NAME/payload.sh"
        [ ! -f "$PAYLOAD_PATH" ] && { ERROR_DIALOG "Payload не найден: $PAYLOAD_PATH"; exit 1; }
