...
command -v log >/dev/null 2>&1 || log() { echo "[$(date '+%H:%M:%S')] $1"; }

PAYLOAD_NAME="GhostСеть"
# подключаем оригинальную библиотеку, если есть
[ -f /root/payloads/library/nullsec-lib.sh ] && source /root/payloads/library/nullsec-lib.sh 2>/dev/null
...
# своя функция log, если не определена
log() {
...
if ! ifconfig "$GHOST_INTERFACE" >/dev/null 2>&1; then
    log "[!] Интерфейс $GHOST_INTERFACE не найден. Проверьте подключение адаптера."
    exit 1
fi
