...
case $? in $DUCKYSCRIPURL сервера C2DUCKYSCRIPT_REJECTED) C2_URL="" ;; esac
[ -z "$C2_URL" ] && { ERROR_DIALOG "C2 URL required!"; exit 1; }
[ "$C2_URL" = "https://c2.example.coТребуется URL C2{ ERROR_DIALOG "Configure a real C2 URL!"; exit 1; }
Настройте реальный URL C2
AUTH_TOKEN=$(TEXT_PICKER "Auth token:" "")
case $? in $DUCKYSCRIPT_CAТокен аутентификации:" "")
...
INTERVAL_MODE=$(NUMBER_PICKER "ИнтервалYSCRIPT_REJECTED) INTERVAL_MODE=2 ;; esac
...
case $INTERVAL_MODE in
    ...
    5) INTERVAL=$(NUMBER_PICKER "Seconds:" 300)
       case $? in $DUCKYSCRIPT_CAСекунды|$DUCKYSCRIPT_REJECTED) INTERVAL=300 ;; esac ;;
    *) INTERVAL=300 ;;
esac
...
case $? in $DUCKYSCRIPT_ДжиттерLED|$DUCKYSCRIPT_REJECTED) JITTER=20 ;; esac
...
MAX_RUNTIME=$(NUMBER_PICKER "Макс время работы (часыREJECTED) MAX_RUNTIME=24 ;; esac
...
Нажмите OK для активацииSCRIPT_USER_CONFIRMED" ] && exit 0
...
SPINNБикон C2 активирован: $BEACON_ID"
...
if kill -0 "$BEACON_PID" 2>/dev/null; then
    ...
    PROMPБикон активен (PID: $BEACON_PID)"
