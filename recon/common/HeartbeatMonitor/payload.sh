...
# Author: NullSec
# Description: Непрерывный мониторинг здоровья для долгих операций
...
while [ $(date +%s) -lt $END_TIME ]; do
    ...
    # --- Состояние WiFi интерфейсов ---
    WLAN0_STATE=$(cat /sys/class/net/$IFACE/operstate 2>/dev/null || echo "отсутствует")
    WLAN1_STATE=$(cat /sys/class/net/wlan1/operstate 2>/dev/null || echo "отсутствует")
    ...
    # --- Показ предупреждения на экране ---
    if [ -n "$ALERT_MSG" ]; then
...
мониторинга."
    ...
    # --- Статус обновление каждые 20 проверок ---
...
Статус: $GRADE
CPU: ${CPU_TEMP}C (пик:${TEMP_PEAK}C)
ПАМ: ${MEM_PCT}% (пик:${MEM_PEAK}%)
Нагрузка: $LOAD_1
