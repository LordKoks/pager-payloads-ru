...
# Title: Loot Sync
# Author: NullSec
# Description: Синхронизирует все собранные файлы лута на USB
# Category: nullsec/exfiltration

LOOT_DIR="/mmc/nullsec/lootsync"
mkdir -p "$LOOT_DIR"

PROMPT "СИНХРОНИЗАЦИЯ ЛУТА

Копирует весь собранный лут
на USB-накопитель для безопасного
оффлайн-доступа.
...
# Try to detect and mount if не найден
if [ -z "$USB_MOUNT" ]; then
...
[ -z "$USB_MOUNT" ] && { ERROR_DIALOG "USB-накопитель не обнаружен!

Вставьте USB и убедитесь,
что он смонтирован.
...
LOG "USB обнаружен: $USB_MOUNT ($USB_FREE свободно)"

PROMPT "USB ОБНАРУЖЕН

Монтировано: $USB_MOUNT
Всего: $USB_TOTAL
Свободно: $USB_FREE

Источник: /mmc/nullsec/

Нажмите ОК для выбора режима."

PROMPT "РЕЖИМ СИНХРОНИЗАЦИИ:

1. Полная синхронизация (весь лут)
2. Только новые файлы (дельта)
3. Лут конкретного payload-а
4. Сжатый архив
...
if [ "$MODE" -eq 3 ]; then
    PAYLOAD_NAME=$(TEXT_PICKER "Имя payload-а:" "datavacuum")
    case $? in $DUCKYSCRIPT_CANCELLED|$DUCKYSCRIPT_REJECTED) PAYLOAD_NAME="" ;; esac
    [ -z "$PAYLOAD_NAME" ] && { ERROR_DIALOG "Не указано имя payload-а!"; exit 1; }
    SRC_PATH="/mmc/nullsec/$PAYLOAD_NAME"
    [ ! -d "$SRC_PATH" ] && { ERROR_DIALOG "Путь не найден:
$SRC_PATH"; exit 1; }
fi
...
resp=$(CONFIRMATION_DIALOG "НАЧАТЬ СИНХРОНИЗАЦИЮ?

Источник: $SRC_PATH
Файлов: $SRC_FILES
Размер: $SRC_SIZE
Режим: $MODE
USB свободно: $USB_FREE

Нажмите ОК для синхронизации.")
...
LOG "Синхронизация лута начата"
SPINNER_START "Синхронизирую лут на USB..."
...
LOG "Синхронизация завершена: $COPIED файлов в $DEST_DIR"

echo "[$TIMESTAMP] Mode=$MODE Files=$COPIED Size=$DEST_SIZE Dest=$DEST_DIR" >> "$SYNC_LOG"

PROMPT "СИНХРОНИЗАЦИЯ ЗАВЕРШЕНА

Скопировано файлов: $COPIED
Размер: $DEST_SIZE
Папка: $DEST_DIR

Осталось на USB: $(df -h "$USB_MOUNT" | tail -1 | awk '{print $4}')
Лог: $SYNC_LOG

Можно безопасно извлечь USB."
