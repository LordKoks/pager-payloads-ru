#!/bin/bash
. /root/payloads/library/nullsec-iface.sh 2>/dev/null || . "$(dirname "$0")/../../../lib/nullsec-iface.sh"
. /root/payloads/library/nullsec-ui.sh
# Title: Тест скорости
# Author: NullSec
# Description: Тестирование скорости интернет-соединения через Pineapple
# Category: nullsec/utility

LOOT_DIR="/mmc/nullsec/speedtest"
mkdir -p "$LOOT_DIR"

PROMPT "ТЕСТ СКОРОСТИ

Проверка скорости интернета
через данное устройство.

Возможности:
- Тест скорости скачивания
- Оценка скорости загрузки
- Измерение задержки (пинг)
- Несколько тестовых серверов
- Сохранение истории тестов

Нажми OK для настройки."

# Проверка подключения
SPINNER_START "Проверка соединения..."

GATEWAY=$(ip route | grep default | awk '{print $3}' | head -1)
if [ -z "$GATEWAY" ]; then
    SPINNER_STOP
    ERROR_DIALOG "Нет подключения к интернету!

Не найден шлюз по умолчанию.
Проверьте настройки сети."
    exit 1
fi

if ! ping -c 2 -W 3 8.8.8.8 >/dev/null 2>&1; then
    SPINNER_STOP
    ERROR_DIALOG "Нет доступа в интернет!

Шлюз доступен, но нет
подключения к внешней сети."
    exit 1
fi

SPINNER_STOP

PROMPT "РЕЖИМ ТЕСТИРОВАНИЯ:

1. Быстрый тест (маленький)
2. Стандартный тест
3. Расширенный тест
4. Только задержка (пинг)

Выбери режим:"

TEST_MODE=$(NUMBER_PICKER "Режим (1-4):" 2)
case $? in $DUCKYSCRIPT_CANCELLED|$DUCKYSCRIPT_REJECTED) TEST_MODE=2 ;; esac

# Настройка параметров в зависимости от режима
case $TEST_MODE in
    1) TEST_SIZE=1048576;  LABEL="Быстрый (1 МБ)";  ITERATIONS=1 ;;
    2) TEST_SIZE=5242880;  LABEL="Стандартный (5 МБ)"; ITERATIONS=3 ;;
    3) TEST_SIZE=10485760; LABEL="Расширенный (10 МБ)"; ITERATIONS=5 ;;
    4) TEST_SIZE=0;        LABEL="Только задержка";  ITERATIONS=10 ;;
esac

resp=$(CONFIRMATION_DIALOG "ЗАПУСТИТЬ ТЕСТ СКОРОСТИ?

Режим: $LABEL
Повторов: $ITERATIONS

Тест будет использовать трафик.

Подтверждаешь?")
[ "$resp" != "$DUCKYSCRIPT_USER_CONFIRMED" ] && exit 0

TIMESTAMP=$(date +%Y%m%d_%H%M)
RESULT_FILE="$LOOT_DIR/speedtest_$TIMESTAMP.log"

LOG "Запуск теста скорости..."

# Тест задержки
SPINNER_START "Измерение задержки..."
LATENCY_TOTAL=0
LATENCY_COUNT=0
for target in 8.8.8.8 1.1.1.1 208.67.222.222; do
    RTT=$(ping -c 5 -W 2 "$target" 2>/dev/null | tail -1 | awk -F'/' '{print $5}')
    if [ -n "$RTT" ]; then
        LATENCY_TOTAL=$(awk "BEGIN{print $LATENCY_TOTAL + $RTT}")
        LATENCY_COUNT=$((LATENCY_COUNT + 1))
    fi
done
if [ $LATENCY_COUNT -gt 0 ]; then
    AVG_LATENCY=$(awk "BEGIN{printf \"%.1f\", $LATENCY_TOTAL / $LATENCY_COUNT}")
else
    AVG_LATENCY="N/A"
fi
SPINNER_STOP

echo "=== Результаты теста скорости ===" > "$RESULT_FILE"
echo "Дата: $(date)" >> "$RESULT_FILE"
echo "Режим: $LABEL" >> "$RESULT_FILE"
echo "Средняя задержка: ${AVG_LATENCY} мс" >> "$RESULT_FILE"

if [ $TEST_MODE -ne 4 ]; then
    # Тест скорости скачивания
    SPINNER_START "Тестирование скорости скачивания..."
    TOTAL_SPEED=0
    SPEED_COUNT=0
    TEST_URLS="http://speedtest.tele2.net/1MB.zip http://proof.ovh.net/files/1Mb.dat"
    for i in $(seq 1 $ITERATIONS); do
        for url in $TEST_URLS; do
            SPEED=$(wget --no-check-certificate -O /dev/null "$url" 2>&1 | \
                grep -oE '[0-9.]+ [KMG]B/s' | tail -1)
            if [ -n "$SPEED" ]; then
                NUM=$(echo "$SPEED" | awk '{print $1}')
                UNIT=$(echo "$SPEED" | awk '{print $2}')
                case "$UNIT" in
                    KB/s) KBS=$NUM ;;
                    MB/s) KBS=$(awk "BEGIN{print $NUM * 1024}") ;;
                    GB/s) KBS=$(awk "BEGIN{print $NUM * 1048576}") ;;
                    *) KBS=0 ;;
                esac
                TOTAL_SPEED=$(awk "BEGIN{print $TOTAL_SPEED + $KBS}")
                SPEED_COUNT=$((SPEED_COUNT + 1))
            fi
            break
        done
    done
    if [ $SPEED_COUNT -gt 0 ]; then
        AVG_DL=$(awk "BEGIN{printf \"%.1f\", $TOTAL_SPEED / $SPEED_COUNT}")
        AVG_DL_MBPS=$(awk "BEGIN{printf \"%.2f\", ($TOTAL_SPEED / $SPEED_COUNT) * 8 / 1024}")
    else
        AVG_DL="N/A"; AVG_DL_MBPS="N/A"
    fi
    SPINNER_STOP

    echo "Скачивание: ${AVG_DL} КБ/с (~${AVG_DL_MBPS} Мбит/с)" >> "$RESULT_FILE"

    PROMPT "РЕЗУЛЬТАТЫ ТЕСТА СКОРОСТИ

Задержка: ${AVG_LATENCY} мс
Скачивание: ${AVG_DL} КБ/с
  (~${AVG_DL_MBPS} Мбит/с)

Результаты сохранены в:
$RESULT_FILE

Нажми OK для выхода."
else
    PROMPT "РЕЗУЛЬТАТЫ ТЕСТА ЗАДЕРЖКИ

Средняя задержка: ${AVG_LATENCY} мс

Тестировалось на:
- Google DNS
- Cloudflare DNS
- OpenDNS

Сохранено в: $RESULT_FILE

Нажми OK для выхода."
fi