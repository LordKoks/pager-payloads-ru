#!/bin/bash
# Title: Цифровой отпечаток устройства
# Author: bad-antics
# Description: Определяет типы устройств по MAC-адресам и probe-запросам
# Category: nullsec/recon

# FIX: правильный PATH для запуска из UI
export PATH=/usr/sbin:/sbin:/bin:/mmc/usr/sbin:/mmc/usr/bin:$PATH

# FIX: подстановка функций UI, если они отсутствуют
...
# Автоопределение интерфейса Wi-Fi (wlan1mon/mon0)
...
resp=$(CONFIRMATION_DIALOG "НАЧАТЬ СКАНИРОВАНИЕ?
...
# Генерация отчёта
echo "==========================================" > "$REPORT"
echo "       NULLSEC ОПРЕДЕЛЕНИЕ УСТРОЙСТВ       " >> "$REPORT"
echo "==========================================" >> "$REPORT"
echo "Время сканирования: $(date)" >> "$REPORT"
echo "" >> "$REPORT"

# Извлечение всех MAC-адресов
...
echo "--- ИДЕНТИФИКАЦИЯ УСТРОЙСТВ ---" >> "$REPORT"
echo "" >> "$REPORT"

while read MAC; do
    PREFIX=$(echo "$MAC" | cut -d':' -f1-3 | tr '[:lower:]' '[:upper:]')
    
    # Префиксы OUI для разных вендоров
    case $PREFIX in
        ...
        "00:1B:21"|"00:1C:BF"|"00:1D:E0"|"00:1E:64"|"00:1F:3B"|"00:21:5C"|"00:22:FA"|"00:24:D6"|"3C:97:0E"|"5C:51:4F"|"64:D4:DA"|"80:86:F2"|"88:53:2E"|"A0:88:B4"|"C8:0A:A9"|"F4:8E:38")
            ...
            ;;
        *)
            echo "OTHER: $MAC (Неизвестный вендор)" >> "$REPORT"
            OTHER=$((OTHER + 1))
            ;;
    esac
...
echo "==========================================" >> "$REPORT"
echo "ИТОГИ:" >> "$REPORT"
echo "  Устройства Apple:      $APPLE" >> "$REPORT"
echo "  Устройства Samsung:    $SAMSUNG" >> "$REPORT"
echo "  Устройства Google:     $GOOGLE" >> "$REPORT"
echo "  Устройства Microsoft:  $MICROSOFT" >> "$REPORT"
echo "  Устройства Amazon:     $AMAZON" >> "$REPORT"
echo "  Устройства Intel:      $INTEL" >> "$REPORT"
echo "  Прочие/Неизвестные:    $OTHER" >> "$REPORT"
echo "==========================================" >> "$REPORT"
