...
[ $HAS_WASH -eq 0 ] && [ $HAS_REAVER -eq 0 ] && {
    ERROR_DIALOG "wash/reaver не найден!
...
WPS_MODE=$(NUMBER_PICKER "Режим (1-4):" 1)
...
echo "=======================================" > "$REPORT"
echo "      СКАНИРОВАНИЕ WPS NULLSEC" >> "$REPORT"
echo "=======================================" >> "$REPORT"
echo "" >> "$REPORT"
...
if [ "$WPS_MODE" -ge 3 ] && [ $HAS_REAVER -eq 1 ]; then
    echo "" >> "$REPORT"
    echo "--- ОЦЕНКА УЯЗВИМОСТЕЙ ---" >> "$REPORT"
    echo "" >> "$REPORT"
