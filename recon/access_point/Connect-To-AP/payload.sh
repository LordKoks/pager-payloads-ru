...
# MetaPayload is optional, but if установлен, you will have the option to update the global TARGET_SUBNET on connection
...
if [ -n "$AP_FREQ" ]; then
    if [ "$AP_FREQ" -ge 5000 ]; then
        ALERT "Ошибка: поддерживается только 2.4 ГГц."
...
if [ -f "$REMEMBERED_APS" ]; then
    ...
    if [ -n "$SAVED_ENTRY" ]; then
        ...
        LOG "Найдено saved credentials for $TARGET_SSID"
    fi
fi
...
if WIFI_CONNECT wlan0cli "$TARGET_SSID" "$NEW_ENC" "$WIFI_PASSWORD" "$_RECON_SELECTED_AP_BSSID"; then
    ...
    if [ "$NEW_ENC" != "open" ] && [ "$WIFI_PASSWORD" != "$SAVED_PASSWORD" ]; then
        resp=$(CONFIRMATION_DIALOG "Сохранить пароль для $TARGET_SSID?")
        case $? in
    ...
    if [ -n "$IP_ADDR" ]; then
        ...
        # === Update MetaPayload TARGET_SUBNET if MetaPayload is установлен ===
    ...
    ALERT "Ошибка подключения к $TARGET_SSID"
