...
case $SOURCE_MODE in
    1) # Scan
        ...
        PROMPT "Найдено $NET_COUNT networks"
        
        TARGET_NUM=$(NUMBER_PICKER "Сеть #:" 1)
        ...
        ;;
    2) # Manual
        SOURCE_SSID=$(TEXT_PICKER "Source SSID:" "MyСеть")
...
case $SSID_MODE in
    ...
    2) # Clone nearby
        ...
        PROMPT "Найдено $CLONE_COUNT networks to clone"
...
if ! ping -c 1 -W 2 8.8.8.8 >/dev/null 2>&1; then
    ERROR_DIALOG "Ошибка подключения к $SOURCE_SSID
...
LOG "Подключено! Запускаю хотспот..."
...
Source: $SOURCE_SSID ✓
Hotspot: $HOTSPOT_SSID
Password: $([ \"$SEC_MODE\" -eq 2 ] && echo $HOTSPOT_PASS || echo 'Нет (открытая)')
