...
ui_prompt_ack() {
  PROMPT "${1:-Done. Нажмите любую кнопку.}" >/dev/null 2>&1 || true
...
# ----------------------------
CONFIRMATION_DIALOG "Требуется авторизация" \
...
СЕТЬ_CIDR="$(ip -4 route show dev "$WIFI_IF" 2>/dev/null | awk '$1 ~ /\/[0-9]+/ && $0 ~ /proto kernel/ {print $1; exit}' || true)"
[ -z "$СЕТЬ_CIDR" ] && СЕТЬ_CIDR="$CIDR"

СЕТЬ_IP="${СЕТЬ_CIDR%/*}"
СЕТЬ_PREFIX="${СЕТЬ_CIDR#*/}"
...

{
  ...
  echo "СЕТЬ_CIDR=$СЕТЬ_CIDR"
  echo "СЕТЬ_IP=$СЕТЬ_IP"
  echo "СЕТЬ_PREFIX=$СЕТЬ_PREFIX"
  echo "BASE24=$BASE24"
...
log_both "Connected: $WIFI_IF CIDR=$CIDR GW=${GATEWAY:-<none>} NET=$СЕТЬ_CIDR"
...
if [ "${СЕТЬ_PREFIX:-}" = "24" ] && [ -n "${BASE24:-}" ]; then
  TARGET_CIDR="${BASE24}0/24"
else
  TARGET_CIDR="$СЕТЬ_CIDR"
fi
